...
// TODO: Проверить работоспособность и раскидать логи
func (u User) UpdateUserPermissionsByRoles() {
	db := database.Instant()
	if len(u.Roles) == 0 {
		db.Model(&User{}).Association("Roles").Find(&u.Roles)
		if len(u.Roles) == 0 {
			return
		}
	}

	var permissions map[Group]string
	var groups []Group

	for _, role := range u.Roles {
		var perm []RoleGroupPermissions
		db.Model(&RoleGroupPermissions{}).Where("role_id = ?", role.ID).Preload("Group.ParentGroup").Find(&perm)
		for _, p := range perm {
